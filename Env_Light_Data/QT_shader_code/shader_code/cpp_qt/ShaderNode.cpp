...
// helper functions
void ShaderNode::printShaderInfoLog(GLuint obj) {
  int infoLogLength = 0;
  int returnLength = 0;
  char* infoLog;
...
void ShaderNode::printProgramInfoLog(GLuint obj) {
  int infoLogLength = 0;
  int returnLength = 0;
  char* infoLog;
...
bool ShaderNode::checkFramebufferStatus() {
  GLenum status;
...
std::string ShaderNode::findAttributeInComments(const std::string& line, const std::string& attr) const
{
...
int ShaderNode::strToBendFunc(const std::string& inputStr) const {
...
bool ShaderNode::beforeGLSLversion1_2(const std::string &code) const {
  const std::vector<string>& lines = StringTools::split(code, "\n");
  for (int i = 0; i < int(lines.size()); i++) {
    string line = lines[i];
    const std::vector<string>& lineParts = StringTools::split(line);
