...
void resamplePts_and_compNormal(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_init,
  ...
  for (int nIndex = 0; nIndex < (mls_points).points.size(); nIndex++) {
    ...
    bool is_match = false;
    n_eigen_current =
        iterate_all_pts(n_eigen_current, cloud_eigen,
                        pt_mls, // assign pt_mls's normal to eigen map's pixel
                        is_match);
    ...
    //		init_normal_map.at<cv::Vec3d>(row_id, col_id)[0] =
    //normal_new(2)*0.5+0.5; 		init_normal_map.at<cv::Vec3d>(row_id, col_id)[1] =
    //normal_new(1)*0.5+0.5; 		init_normal_map.at<cv::Vec3d>(row_id, col_id)[2] =
    //normal_new(0)*0.5+0.5;
