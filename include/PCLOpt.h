...
void resamplePts_and_compNormal(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered, std::vector<Eigen::Vector3d> cloud_eigen, cv::Mat& init_normal_map);

int iterate_all_pts(int n_eigen_current, const std::vector<Eigen::Vector3d>& cloud_eigen, Eigen::Vector3d pt_mls, bool& is_match);
...
public:
	void storeNormal( pcl::PointCloud<pcl::Normal>::Ptr cloud_mls_normal, int pointIdx, Mat &dst, int u, int  v  ){
	...
	void convolve(Mat& src, Mat &dst, int& k_w, int& k_h, string& paddingType,const Eigen::Matrix<double,3,3>& K_)
	{
				...
				for (int i = u-pad_rows; i <= u+pad_rows; i++) // row
				{
					for (int j = v-pad_cols; j <= v+pad_cols; j++)// col
					{
						...
						if (i>=0 && i<= src.rows && j >=0 && j<= src.cols){
							...
							cloud_patch->push_back(pcl::PointXYZ(point.val[0],point.val[1],point.val[2]));
						}
//						Vec3f point(p_3d_no_d_path.at<Vec3f>(i + pad_rows, j + pad_cols).val[0], p_3d_no_d_path.at<Vec3f>(i + pad_rows, j + pad_cols).val[1], p_3d_no_d_path.at<Vec3f>(i + pad_rows, j + pad_cols).val[2]);
...
//				mls.setNumberOfThreads(4);
				mls.setSearchRadius (1.0); // 307200 pts  tune the parameter !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
				mls.process (mls_points);


				pcl::KdTreeFLANN<pcl::PointXYZ> kdtree_;
				pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_mls (new pcl::PointCloud<pcl::PointXYZ> ());
				pcl::PointCloud<pcl::Normal>::Ptr cloud_mls_normal (new pcl::PointCloud<pcl::Normal> ());
				for (int nIndex = 0; nIndex < mls_points.size(); ++nIndex) {
					cloud_mls->push_back(pcl::PointXYZ(mls_points.points[nIndex].x,mls_points.points[nIndex].y,mls_points.points[nIndex].z));
					cloud_mls_normal->push_back(pcl::Normal(mls_points.points[nIndex].normal_x,mls_points.points[nIndex].normal_y,mls_points.points[nIndex].normal_z));

				}
				if (cloud_mls->size()==0 || isnan(cloud_mls->points[0].x)){ continue;}
				...
				if ( kdtree_.nearestKSearch (currentPoint, K, pointIdxKNNSearch, pointKNNSquaredDistance) > 0 ){

//					double n_x = (*cloud_mls_normal)[ pointIdxKNNSearch[0] ].normal_x;
//					double n_y = (*cloud_mls_normal)[ pointIdxKNNSearch[0] ].normal_y;
//					double n_z =(*cloud_mls_normal)[ pointIdxKNNSearch[0] ].normal_z;
//
//					Vec3f normal_new(n_x, n_y, n_z);
//					normal_new = normalize(normal_new);
//
...
//					if (v+1<=640){storeNormal(cloud_mls_normal,index_+1,dst,u,v+1);}
//					if (v+2<=640){storeNormal(cloud_mls_normal,index_+2,dst,u,v+2 );}
...
//					if (v+1<=640) {storeNormal(cloud_mls_normal,index_+8,dst,u+1,v+1);}
//					if (v+1<=640) {storeNormal(cloud_mls_normal,index_+9,dst,u+1,v+2);}
...
//					if (v+1<=640) {storeNormal(cloud_mls_normal,index_+15,dst,u+1,v+1);}
//					if (v+1<=640) {storeNormal(cloud_mls_normal,index_+16,dst,u+1,v+2);}
